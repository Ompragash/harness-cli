...
func applyRepository(c *cli.Context) error {
	...
	if !entityExists {
		...
		/*repoPUTUrl := GetUrlWithQueryParams("", baseURL,
		fmt.Sprintf(GITOPS_REPOSITORY_ENDPOINT+"/%s", identifier),
		queryParams)*/
		...
		newRepoPayload := createRepoPayload(requestBody)
...
func createRepoPayload(requestBody map[string]interface{}) HarnessRepository {
	newRepo := HarnessRepository{Repo: Repo{
		Name:           valueToString(GetNestedValue(requestBody, "gitops", "repo", "name").(string)),
